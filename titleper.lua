...
local patched = true
local myoutfile = "/tmp/mpv-favicon.png"
...
local streamerable =
"iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAMAAAAoLQ9TAAABU1BMVEUPkPoPkPoPkPoPkPoPkPoP" ..
...
function get_uploader(url, uploader)
  return os.capture('youtube-dl -j "' .. url .. '"| jq -r .' .. uploader)
end
...
function on_loaded()
  ...
  if string.find(title, "/") then
    ...
    utils.subprocess{ args = {"sleep", "0.1"} }
    mp.set_property("file-local-options/title", title)
  end

  local filepath = mp.get_property "path"

  if filepath:find "^http" == nil then return end
  ...
  elseif filepath:find "streamable.com" then writebase64(streamable) end

  if file_exists(myoutfile) then
    msg.warn "website icon use"
    utils.subprocess{ args = {"sh", "-c",
      "xseticon -id $(xdotool search --sync --onlyvisible --pid $PPID) " .. myoutfile } }
    if json_uploader and not patched then
      local uploader = get_uploader(filepath, json_uploader)
      if uploader then
        local newtitle = title .. ' @' .. uploader
        msg.warn("filepath: " .. filepath)
        msg.warn("uploader: " .. uploader)
